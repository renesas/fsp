...
WIFIReturnCode_t WIFI_On (void)
{
    ...
    ret = rm_wifi_da16xxx_open(&g_wifi_da16xxx_cfg);
    if (!ret)
    {
...
WIFIReturnCode_t WIFI_Off (void)
{
    ...
    ret = (int32_t) rm_wifi_da16xxx_close();
    if (!ret)
    {
        xRetVal = eWiFiSuccess;
    }
...
WIFIReturnCode_t WIFI_ConnectAP (const WIFINetworkParams_t * const pxNetworkParams)
{
    ...
    ret = (int32_t) rm_wifi_da16xxx_connect((char *) pxNetworkParams->ucSSID,
                                            pxNetworkParams->xSecurity,
                                            pxNetworkParams->xPassword.xWPA.cPassphrase,
                                            WIFI_DA16XXX_TKIP_AES_ENC_TYPE);
    if (!ret)
    {
        xRetVal = eWiFiSuccess;
    }
...
WIFIReturnCode_t WIFI_Disconnect (void) {
    ...
    ret = (int32_t) rm_wifi_da16xxx_disconnect();
    if (!ret)
    {
        xRetVal = eWiFiSuccess;
    }
...
WIFIReturnCode_t WIFI_Scan (WIFIScanResult_t * pxBuffer, uint8_t ucNumNetworks)
{
    ...
    err = rm_wifi_da16xxx_scan(pxBuffer, ucNumNetworks);
...
WIFIReturnCode_t WIFI_Ping (uint8_t * pucIPAddr, uint16_t usCount, uint32_t ulIntervalMS)
{
    ...
    ret = (int32_t) rm_wifi_da16xxx_ping(pucIPAddr, usCount, ulIntervalMS);
...
WIFIReturnCode_t WIFI_GetIPInfo (WIFIIPConfiguration_t * pxIPInfo) {
    ...
    ret = (int32_t) rm_wifi_da16xxx_network_info_get(pxIPInfo->xIPAddress.ulAddress,
...
WIFIReturnCode_t WIFI_GetMAC (uint8_t * pucMac) {
    ...
    ret = (int32_t) rm_wifi_da16xxx_mac_addr_get(pucMac);
    FSP_ERROR_RETURN(FSP_SUCCESS == ret, eWiFiFailure);
...
WIFIReturnCode_t WIFI_GetHostIP (char * pcHost, uint8_t * pucIPAddr) {
    ...
    ret = (int32_t) rm_wifi_da16xxx_dns_query(pcHost, pucIPAddr);
    FSP_ERROR_RETURN(FSP_SUCCESS == ret, eWiFiFailure);
