...
#define CTSU_TUNING_VALUE_MUTUAL           (10240)
 ...
 #define CTSU_SST_RECOMMEND                (0x1F) // The value of SST should be fixed
 ...
 #define CTSU_REG_TRIMA                    (*(uint32_t *) 0x407EC3A4)
 #define CTSU_REG_TRIMB                    (*(uint32_t *) 0x407EC3A8)
...
/* Macro definitions for correction */
 ...
 #define CTSU_CORRECTION_OFFSET_UNIT       (120)   // (CTSU_CORRECTION_STD_VAL / 256) * 2
 ...
 #define CTSU_CORRECTION_BIT7              (0x0080)
  ...
  #define CTSU_CORRCFC_POINT_NUM           (5)                                // number of table
  ...
  #define CTSU_CALIB_REF                   (6140000 / CTSU_CFG_VCC_MV)        // 1.5V Reference value (4096 * 1500)
...
static void ctsu_noise_reduction(ctsu_instance_ctrl_t * const p_instance_ctrl);
...
static void ctsu_correction_data_get(ctsu_instance_ctrl_t * const p_instance_ctrl);

  #if (CTSU_CFG_CALIB_RTRIM_SUPPORT == 1)
static void ctsu_correction_calib_rtrim(ctsu_instance_ctrl_t * const p_instance_ctrl);
...
static uint8_t       g_ctsu_tuning_complete[CTSU_CFG_NUM_SELF_ELEMENTS + CTSU_CFG_NUM_MUTUAL_ELEMENTS];
...
fsp_err_t R_CTSU_Open (ctsu_ctrl_t * const p_ctrl, ctsu_cfg_t const * const p_cfg)
{
    ...
    if (CTSU_MODE_MUTUAL_CFC_SCAN != p_cfg->md)
    {
        ...
        if (CTSU_MODE_CORRECTION_SCAN == p_cfg->md)
        {
            ...
            ctsu_correction_calib_rtrim(p_instance_ctrl);
  #endif
    ...
    R_CTSU->CTSUSUCLK0       = (uint16_t) (CTSU_CFG_SUMULTI0 << 8) | CTSU_SUADJ_RECOMMEND;
    R_CTSU->CTSUSUCLK1       = (uint16_t) (CTSU_CFG_SUMULTI1 << 8) | CTSU_SUADJ_RECOMMEND;
    R_CTSU->CTSUSUCLK2       = (uint16_t) (CTSU_CFG_SUMULTI2 << 8) | CTSU_SUADJ_RECOMMEND;
 ...
 * @retval FSP_ERR_CTSU_NOT_GET_DATA    The previous data does not been getted by DataGet.
 **********************************************************************************************************************/
...
fsp_err_t R_CTSU_DataGet (ctsu_ctrl_t * const p_ctrl, uint16_t * p_data)
{
    ...
    if (CTSU_MODE_CORRECTION_SCAN == p_instance_ctrl->p_ctsu_cfg->md)
    {
        ctsu_correction_data_get(p_instance_ctrl);
        p_instance_ctrl->state = CTSU_STATE_IDLE;
    ...
    if (CTSU_STATE_SCANNED == p_instance_ctrl->state)
    {
        ...
        if (CTSU_TUNING_COMPLETE == p_instance_ctrl->tuning)
        {
            ...
            ctsu_noise_reduction(p_instance_ctrl);
        }
        else
        {
...
fsp_err_t ctsu_transfer_open (ctsu_instance_ctrl_t * const p_instance_ctrl)
{
    ...
    p_info->length         = (uint16_t) (p_instance_ctrl->num_elements * CTSU_CFG_NUM_SUMULTI);
 #endif
 #if (BSP_FEATURE_CTSU_VERSION == 1)
    ...
    for (element_id = 0; element_id < p_instance_ctrl->num_elements; element_id++)
    {
        if (0 == *(p_instance_ctrl->p_tuning_complete + element_id))
        {
            ...
            for (i = 0; i < CTSU_CFG_NUM_SUMULTI; i++)
            {
                ...
                if (CTSU_MODE_SELF_MULTI_SCAN == p_instance_ctrl->p_ctsu_cfg->md)
                {
                    corr_data[i] = p_instance_ctrl->p_self_raw[element_top + i];
                }
 #endif
 #if (CTSU_CFG_NUM_MUTUAL_ELEMENTS != 0)
                if (CTSU_MODE_MUTUAL_FULL_SCAN == p_instance_ctrl->p_ctsu_cfg->md)
                {
                    corr_data[i] = p_instance_ctrl->p_mutual_raw[(element_top * 2) + (i * 2)];
                }
 #endif
...
/***********************************************************************************************************************
 * ctsu_noise_reduction
 ***********************************************************************************************************************/
void ctsu_noise_reduction (ctsu_instance_ctrl_t * const p_instance_ctrl)
{
    uint16_t   element_id;
    ...
    for (element_id = 0; element_id < p_instance_ctrl->num_elements; element_id++)
    {
        ...
        if (CTSU_MODE_SELF_MULTI_SCAN == (CTSU_MODE_MUTUAL_FULL_SCAN & p_instance_ctrl->p_ctsu_cfg->md))
        {
            ...
            {
                ctsu_moving_average(p_average_data,
                                    *(p_instance_ctrl->p_self_work + element_id),
                                    p_instance_ctrl->average);
            }
        }
#endif
        ...
        if (CTSU_MODE_MUTUAL_FULL_SCAN == (CTSU_MODE_MUTUAL_FULL_SCAN & p_instance_ctrl->p_ctsu_cfg->md))
        {
            ...
            {
                ctsu_moving_average(p_average_data,
                                    *(p_instance_ctrl->p_mutual_pri_work + element_id),
                                    p_instance_ctrl->average);
            }
            ...
            {
                ctsu_moving_average(p_average_data2,
                                    *(p_instance_ctrl->p_mutual_snd_work + element_id),
                                    p_instance_ctrl->average);
            }
        }
#endif
...
void ctsu_correction_process (ctsu_instance_ctrl_t * const p_instance_ctrl)
{
    ...
    trimb = CTSU_REG_TRIMB;
    for (i = 0; i < CTSU_RANGE_NUM; i++)
    {
        trimb_byte = (trimb >> (i * 8)) & CTSU_CORRECTION_TRIMB_MAX;
        if (trimb_byte & CTSU_CORRECTION_TRIMB_SIGN_BIT)
        {
            /* Plus : bit8(0.50) and bit7(0.25) set 0, bit9(1.00) set 1 */
            error_registance[i] =
                (uint16_t) ((trimb_byte & (uint8_t) (~CTSU_CORRECTION_TRIMB_SIGN_BIT)) | CTSU_CORRECTION_BIT9);
        }
        else
        {
            /* Minus : bit8(0.50) and bit7(0.25) set 1 */
            error_registance[i] = (uint16_t) (trimb_byte | CTSU_CORRECTION_BIT7 | CTSU_CORRECTION_BIT8);
        }

        /* Reverse sign, and 6-bit left shift, 512 to 32768 */
 ...
 ***********************************************************************************************************************/
void ctsu_correction_data_get (ctsu_instance_ctrl_t * const p_instance_ctrl)
{
    uint32_t i;
    uint32_t j;
    ...
    if (g_ctsu_correction_info.update_counter > CTSU_CFG_TEMP_CORRECTION_TIME)
    {
        g_ctsu_correction_info.error_rate[CTSU_RANGE_160UA] =
            (uint16_t) ((g_ctsu_correction_info.ex_base_value << CTSU_SHIFT_AMOUNT) /
                        ((g_ctsu_correction_info.dac_value[7] + g_ctsu_correction_info.dac_value[6]) / 2));
        for (j = 0; j < CTSU_CORRECTION_POINT_NUM; j++)
        {
            g_ctsu_correction_info.real_value[j] =
                (uint16_t) ((g_ctsu_correction_info.dac_value[j] *
                             g_ctsu_correction_info.error_rate[CTSU_RANGE_160UA]) >> CTSU_SHIFT_AMOUNT);
            g_ctsu_correction_info.coefficient[CTSU_RANGE_160UA][j] =
                (uint16_t) (((CTSU_CORRECTION_STD_UNIT * (j + 1)) << CTSU_SHIFT_AMOUNT) /
                            g_ctsu_correction_info.real_value[j]);
            for (i = 0; i < CTSU_RANGE_NUM - 1; i++)
            {
                g_ctsu_correction_info.coefficient[i][j] =
                    (uint16_t) ((g_ctsu_correction_info.coefficient[CTSU_RANGE_160UA][j] *
...
void ctsu_correction_calib_rtrim (ctsu_instance_ctrl_t * const p_instance_ctrl)
{
    ...
    while (!comp)
    {
        ...
        adctdr_ave = (uint16_t) (adctdr_sum / CTSU_CALIB_AVERAGE_TIME);
...
void ctsu_correction_calc (uint16_t * correction_data, uint16_t raw_data, ctsu_correction_calc_t * p_calc)
{
    ...
    if (calc_flag)
    {
        ...
        diff_coefficient =
            (int32_t) (g_ctsu_correction_info.second_coefficient - g_ctsu_correction_info.first_coefficient);
        ...
        {
            for (i = 1; i < CTSU_CORRECTION_POINT_NUM; i++)
            {
                if (((CTSU_CORRECTION_POINT_NUM - 1) == i) ||
                    (cmp_data < g_ctsu_correction_info.real_value[i]))
                {
                    y0 = g_ctsu_correction_info.coefficient[p_calc->range][i];
                    y1 = g_ctsu_correction_info.coefficient[p_calc->range][i - 1];
                    x0 = g_ctsu_correction_info.real_value[i];
                    x1 = g_ctsu_correction_info.real_value[i - 1];
                    break;
                }
            }
        }
        ...
        {
            for (i = 1; i < CTSU_CORRCFC_POINT_NUM; i++)
            {
                if (((CTSU_CORRCFC_POINT_NUM - 1) == i) ||
                    (cmp_data < g_ctsu_corrcfc_info.value[p_calc->cfc][i]))
                {
                    y0 = g_ctsu_corrcfc_info.coefficient[p_calc->cfc][i];
                    y1 = g_ctsu_corrcfc_info.coefficient[p_calc->cfc][i - 1];
                    x0 = g_ctsu_corrcfc_info.value[p_calc->cfc][i];
                    x1 = g_ctsu_corrcfc_info.value[p_calc->cfc][i - 1];
                    break;
                }
            }
        }
        ...
        coefficient = (uint16_t) (y0 - (((y0 - y1) * (x0 - cmp_data)) / (x0 - x1)));
#endif
...
void ctsu_correction_exec (ctsu_instance_ctrl_t * const p_instance_ctrl)
{
 ...
 #if (CTSU_CFG_NUM_SELF_ELEMENTS != 0)
        p_self_work = (p_instance_ctrl->p_self_work + element_id);
        if (CTSU_MODE_SELF_MULTI_SCAN == p_instance_ctrl->p_ctsu_cfg->md)
        {
            ctsu_correction_calc(p_self_work, (p_instance_ctrl->p_self_raw + element_id)->sen, &calc);
        }
 #endif
 #if (CTSU_CFG_NUM_MUTUAL_ELEMENTS != 0)
        p_pri_work = (p_instance_ctrl->p_mutual_pri_work + element_id);
        p_snd_work = (p_instance_ctrl->p_mutual_snd_work + element_id);
        if (CTSU_MODE_MUTUAL_FULL_SCAN == p_instance_ctrl->p_ctsu_cfg->md)
        {
            ctsu_correction_calc(p_pri_work, (p_instance_ctrl->p_mutual_raw + element_id)->pri_sen, &calc);
            ctsu_correction_calc(p_snd_work, (p_instance_ctrl->p_mutual_raw + element_id)->snd_sen, &calc);
        }
 #endif
    }
#endif
  ...
  #endif
 #endif
    multi.calc.range = p_instance_ctrl->range;
    multi.calc.md    = p_instance_ctrl->p_ctsu_cfg->md;
    for (element_id = 0; element_id < p_instance_ctrl->num_elements; element_id++)
    {
        multi.calc.snum = (p_instance_ctrl->p_ctsuwr[(element_id * CTSU_CFG_NUM_SUMULTI)].ctsuso >> 10) & CTSU_SNUM_MAX;
        for (i = 0; i < CTSU_CFG_NUM_SUMULTI; i++)
        {
            ctsuso =
                (p_instance_ctrl->p_ctsuwr[(element_id * CTSU_CFG_NUM_SUMULTI) + i].ctsuso & CTSU_TUNING_MAX);
            multi.offset[i] = (ctsuso * (uint32_t) (CTSU_CORRECTION_OFFSET_UNIT >> multi.calc.range));
        }
        ...
        if (CTSU_MODE_SELF_MULTI_SCAN == p_instance_ctrl->p_ctsu_cfg->md)
        {
            for (i = 0; i < CTSU_CFG_NUM_SUMULTI; i++)
            {
                multi.pri[i] = p_instance_ctrl->p_self_raw[(element_id * CTSU_CFG_NUM_SUMULTI) + i];
                multi.snd[i] = 0;
            }

            if (CTSU_TUNING_COMPLETE == p_instance_ctrl->tuning)
            {
                ctsu_correction_multi(&multi, p_self_work, NULL);
            }
            else
            {
                for (i = 0; i < CTSU_CFG_NUM_SUMULTI; i++)
                {
                    ctsu_correction_calc(&p_instance_ctrl->p_self_raw[(element_id * CTSU_CFG_NUM_SUMULTI) + i],
                                         multi.pri[i],
                                         &multi.calc);
                }
            }
        ...
        if (CTSU_MODE_CURRENT_SCAN == p_instance_ctrl->p_ctsu_cfg->md)
        {
            ctsu_correction_calc(p_self_work, *(p_instance_ctrl->p_self_raw + element_id), &multi.calc);
        }
 #endif
 #if (CTSU_CFG_NUM_MUTUAL_ELEMENTS != 0)
        p_pri_work = (p_instance_ctrl->p_mutual_pri_work + element_id);
        ...
        if (CTSU_MODE_MUTUAL_FULL_SCAN == p_instance_ctrl->p_ctsu_cfg->md)
        {
            for (i = 0; i < CTSU_CFG_NUM_SUMULTI; i++)
            {
                multi.pri[i] = p_instance_ctrl->p_mutual_raw[(element_id * CTSU_MUTUAL_BUF_SIZE) + (i * 2)];
                multi.snd[i] = p_instance_ctrl->p_mutual_raw[(element_id * CTSU_MUTUAL_BUF_SIZE) + (i * 2) + 1];
            }

            if (CTSU_TUNING_COMPLETE == p_instance_ctrl->tuning)
            {
                ctsu_correction_multi(&multi, p_pri_work, p_snd_work);
            }
            else
            {
                for (i = 0; i < CTSU_CFG_NUM_SUMULTI; i++)
                {
                    ctsu_correction_calc(&p_instance_ctrl->p_mutual_raw[(element_id * CTSU_MUTUAL_BUF_SIZE) + (i * 2)],
                                         multi.pri[i],
                                         &multi.calc);
                }
            }
        ...
        if (CTSU_MODE_MUTUAL_CFC_SCAN == p_instance_ctrl->p_ctsu_cfg->md)
        {
            ...
            for (i = 0; i < CTSU_CFG_NUM_SUMULTI; i++)
            {
                multi.pri[i] = p_instance_ctrl->p_mutual_raw[offset + cfc_id + (num_rx * i * 2)];
                multi.snd[i] = p_instance_ctrl->p_mutual_raw[offset + cfc_id + (num_rx * i * 2) + num_rx];
            }

            multi.calc.cfc = cfc_ts_table[cfc_id];
...
void ctsu_correction_multi (ctsu_correction_multi_t * p_multi, uint16_t * p_pri, uint16_t * p_snd)
{
    ...
    for (i = 0; i < CTSU_CFG_NUM_SUMULTI; i++)
    {
        ctsu_correction_calc(&pri_calc[i], p_multi->pri[i], &p_multi->calc);
    }

    for (i = 1; i < CTSU_CFG_NUM_SUMULTI; i++)
    {
        pri_calc[i] =
            (uint16_t) ((((pri_calc[i] + p_multi->offset[i]) * sumulti[0]) / sumulti[i]) - p_multi->offset[0]);
    }

    if (CTSU_MODE_SELF_MULTI_SCAN == p_multi->calc.md)
    {
        for (i = 0; i < CTSU_CFG_NUM_SUMULTI; i++)
        {
            snd_calc[i] = 0;
    ...
    else
    {
        for (i = 0; i < CTSU_CFG_NUM_SUMULTI; i++)
        {
            ctsu_correction_calc(&snd_calc[i], p_multi->snd[i], &p_multi->calc);
        }

        for (i = 1; i < CTSU_CFG_NUM_SUMULTI; i++)
        {
            snd_calc[i] =
                (uint16_t) ((((snd_calc[i] + p_multi->offset[i]) * sumulti[0]) / sumulti[i]) - p_multi->offset[0]);
        }
    }
...
void ctsu_corrcfc_process (ctsu_instance_ctrl_t * const p_instance_ctrl)
{
    ...
    for (j = 0; j < CTSU_CORRCFC_POINT_NUM; j++)
    {
        ...
        ctsu_corrcfc_measurement(p_instance_ctrl, &g_ctsu_corrcfc_info.value[index][j], CTSU_CORRCFC_POINT_NUM);
    }
    ...
    for (i = index; i < (index + g_ctsu_corrcfc_info.num_ts); i++)
    {
        g_ctsu_corrcfc_info.error_rate[i] =
            (uint16_t) ((g_ctsu_corrcfc_info.base_value[i] << CTSU_SHIFT_AMOUNT) /
                        g_ctsu_corrcfc_info.value[i][CTSU_CORRCFC_CENTER_POINT]);

        for (j = 0; j < CTSU_CORRCFC_POINT_NUM; j++)
        {
            ref_temp = (uint32_t) ((CTSU_CORRECTION_STD_UNIT * (j + CTSU_CORRCFC_CENTER_POINT)) << CTSU_SHIFT_AMOUNT);

            g_ctsu_corrcfc_info.value[i][j] =
                (uint16_t) ((g_ctsu_corrcfc_info.value[i][j] * g_ctsu_corrcfc_info.error_rate[i]) >> CTSU_SHIFT_AMOUNT);

            g_ctsu_corrcfc_info.coefficient[i][j] = (uint16_t) (ref_temp / g_ctsu_corrcfc_info.value[i][j]);
        }
    }
