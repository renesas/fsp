    ...
    while (ofdsc < (length - table[0]))
    {
        if (USB_DT_ENDPOINT == table[ofdsc + 1]) /* Search within Interface */
        {
            if (USB_EP_BULK == (table[ofdsc + 3] & USB_EP_TRNSMASK)) /* Bulk Endpoint */
            {
                ...
                R_USB_HmscSmpDrive2Addr (side, &devadr);           /* Get USB Device address -> "devadr.keyword" */

                if (USB_EP_IN == (pdescriptor[2] & USB_EP_DIRMASK))
                {
                    g_usb_hmsc_in_pipe[ip][side] = usb_hstd_make_pipe_reg_info (ip, devadr.keyword, USB_CLASS_HMSC, speed,
                                                             pdescriptor, &g_usb_hmsc_pipe_table[side][USB_PIPE_DIR_IN]);
                }
                else
                {
                    g_usb_hmsc_out_pipe[ip][side] = usb_hstd_make_pipe_reg_info (ip, devadr.keyword, USB_CLASS_HMSC, speed,
...
void usb_hmsc_strg_cmd_complete (usb_utr_t *mess, uint16_t devadr, uint16_t data2)
{
    ...
    ctrl.module_number = (uint8_t)mess->ip; /* Module number setting */
    ctrl.pipe = (uint8_t)mess->keyword; /* Pipe number setting */
    ctrl.device_address = usb_hstd_get_devsel(mess, ctrl.pipe) >> 12;
    ctrl.data_size = 0;
