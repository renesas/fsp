...
void   __iar_Initlocks(void);
  ...
  #define TX_THREAD_CREATE_EXTENSION(thread_ptr)     thread_ptr->tx_thread_iar_tls_pointer = \
    _tx_iar_create_per_thread_tls_area();

  #ifdef TX_PORT_TRUSTZONE_NSC_ENABLE
   #define TX_THREAD_DELETE_EXTENSION(thread_ptr)    do {_tx_iar_destroy_per_thread_tls_area(                         \
...
/* No IAR library support. */
  ...
  #ifdef TX_PORT_TRUSTZONE_NSC_ENABLE
   #define TX_THREAD_DELETE_EXTENSION(thread_ptr)    if (thread_ptr->tx_thread_secure_stack_context) { \
...
/* Cortex-M4, Cortex-M33, and Cortex-M85 have BASEPRI, so BASEPRI can be used to allow high priority interrupts
 * to preempt the scheduler. */

 #if (defined(RENESAS_CORTEX_M4) || defined(RENESAS_CORTEX_M33) || defined(RENESAS_CORTEX_M85)) && TX_PORT_MAX_IPL != 0
