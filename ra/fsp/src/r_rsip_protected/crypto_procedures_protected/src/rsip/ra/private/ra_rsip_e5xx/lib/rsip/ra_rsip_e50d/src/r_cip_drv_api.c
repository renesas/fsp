...
#define OEM_ROT_PK_NOT_WRITTEN  (0xFFFFFFFF)
#define OEM_ROT_PK_NUM          (4)
 ...
 **********************************************************************************************************************/
cip_drv_err_t RootPkErr;
uint32_t *p_RootPkPass;
...
cip_drv_ret_t R_CIP_DRV_CheckImageVersion(const uint32_t image_version, const uint32_t build_num)
{
    ...
    LC[0] = R_PSCU->DLMMON;
    Cmd[0] = bswap_32big(CIP_OEM_KEY_TYPE_OEM_ROOT_PK);

    rootPk = (uint32_t *)CIP_DRV_OEMROOTPKHASH_ADDR3;
    RootPkErr = CIP_DRV_RET_ERR_FAIL;

    for(i = 0;i < OEM_ROT_PK_NUM;i++)
    {
        if(OEM_ROT_PK_NOT_WRITTEN != *rootPk)
        {
        	rsip_ret = r_rsip_p10(LC, Cmd, rootPk);
            ...
            {
                p_RootPkPass = rootPk;
                RootPkErr = CIP_DRV_RET_PASS;

                break;
            }
        }
        rootPk -= ((uint32_t)CIP_DRV_OEMROOTPKHASH_ADDR_SIZE / WORD_BYTE_SIZE);
    }
        ...
        const uint32_t mac_algo, uint32_t* const p_tag)
{
    ...
    volatile cip_drv_ret_t ret_complement;
    cip_drv_ret_t ret_tmp;

    if(CIP_DRV_RET_SUCCESS != RootPkErr)
    {
        ret = CIP_DRV_RET_FAIL;
        return ret;
    }
