/**************************************************************************/
/* This file is a copy of the nx_crypto_ecdh.c file from NetX Duo         */
/* It has been modified to work with Renesas Secure Crypto Engine.        */
/*  \file nx_crypto_ecdh_alt.c                                            */
/**************************************************************************/
...
NX_CRYPTO_HUGE_NUMBER private_key;
    ...
    NX_CRYPTO_PARAMETER_NOT_USED(public_key_len);
    NX_CRYPTO_PARAMETER_NOT_USED(private_key);
    return(sce_nx_crypto_ecdh_key_pair_import(ecdh_ptr, curve, local_private_key_ptr, local_private_key_len, local_public_key_len));
#else
...
{
#if (1U == NETX_SECURE_CRYPTO_NX_CRYPTO_METHODS_ECC_ALT)
    return(sce_nx_crypto_ecdh_private_key_export(ecdh_ptr, local_private_key_ptr, local_private_key_len, actual_local_private_key_len));
#else
UINT                  status;
UINT                  key_size;
UINT                  clen;
...
NX_CRYPTO_EC_POINT    public_key;
    ...
    NX_CRYPTO_PARAMETER_NOT_USED(private_key);
    NX_CRYPTO_PARAMETER_NOT_USED(public_key);
    return(sce_nx_crypto_ecdh_setup(ecdh_ptr,local_public_key_ptr, actual_local_public_key_len, curve));
#else
...
#if (1U == NETX_SECURE_CRYPTO_NX_CRYPTO_METHODS_ECC_ALT)
    NX_CRYPTO_PARAMETER_NOT_USED(scratch_buf_ptr);
    NX_CRYPTO_PARAMETER_NOT_USED(status);
    NX_CRYPTO_PARAMETER_NOT_USED(private_key);
    NX_CRYPTO_PARAMETER_NOT_USED(public_key);
    NX_CRYPTO_PARAMETER_NOT_USED(shared_secret);
    return(sce_nx_crypto_ecdh_compute_secret(ecdh_ptr, share_secret_key_ptr, actual_share_secret_key_len, remote_public_key));
#else
