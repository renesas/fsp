...
tfm_platform_hal_flash_service_write(struct tfm_flash_service_args_t *args,
                                     struct tfm_flash_service_out_t *out)
{
    ...
    if ((flash_address < SECONDARY_PARTITION_START) || ((flash_address + count) > (TOTAL_ROM_SIZE - 1)) ||
...
static enum tfm_platform_err_t
tfm_platform_hal_flash_service_erase(struct tfm_flash_service_args_t *args,
                                     struct tfm_flash_service_out_t *out)
{
    ARM_FLASH_INFO *info;
    info = Driver_CFLASH.GetInfo();
    uint32_t address = args->u.flash_erase.addr;
    uint32_t sector_size = info->sector_size;
    address = address & ~(sector_size - 1);
    uint32_t size = ((args->u.flash_erase.count) * sector_size);

    if ((address < SECONDARY_PARTITION_START) || ((address + size) > (TOTAL_ROM_SIZE)))
    {
        return TFM_PLATFORM_ERR_INVALID_PARAM;
    }
    ...
    {
        out->u.result |= Driver_CFLASH.EraseSector(address);
        address += sector_size;
    }
...
static enum tfm_platform_err_t
tfm_platform_hal_flash_service(const psa_invec  *in_vec,
