...
extern int ecp_load_curve_attributes_sce(const mbedtls_ecp_group * grp, uint32_t * p_curve_type, uint32_t * p_cmd, sce_oem_cmd_t * oem_priv_cmd);
                             ...
                             void * p_rng)
{
    ...
    p_common_buff_32 = mbedtls_calloc((ECC_PUBLIC_KEY_SIZE_BYTES(curve_bytes) / 4) + private_key_size_words, sizeof(uint32_t));
                                 ...
                                 mbedtls_ecp_restart_ctx * rs_ctx)
{
             ...
             mbedtls_mpi_write_binary(&P->Y, (uint8_t *) (p_point_buff_P_32 + ((curve_bytes) / 4)), curve_bytes))
    {
        ret = MBEDTLS_ERR_ECP_ALLOC_FAILED;
            ...
            p_hw_sce_ecc_scalarmultiplication(&curve_type, 
                                              &cmd,
                                              p_integer_buff_m_wrapped_32,
                                              p_point_buff_P_32,
